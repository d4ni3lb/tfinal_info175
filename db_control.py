...
#Mostrar en la grilla todos los locales de la bd
    ...
    query = "SELECT nombre_l AS Local ,ciudad.nombre_c AS Ciudad ,local.direccion, COUNT(nombre) AS Total_empleado FROM local,ciudad LEFT join  empleado on local.fk_id_ciudad = ciudad.id_ciudad and empleado.fk_id_local = local.id_local GROUP BY nombre_l" #Definimos la query
